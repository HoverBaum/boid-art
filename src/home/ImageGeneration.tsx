...
export const ImageGeneration = () => {
  ...
  return (
    <div>
      <Link
        className="absolute top-6 z-10 right-6"
        style={linkStyle}
        href="/results"
      >
        ART GALLERY --{'>'}
        ...
        <Container>
          ...
          {currentEvent !== 'none' && (
            <div>
              <textarea
                id="message"
                className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50/70 bg-op rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
        ...
        <Container>
          <div className="sm:w-[512px] sm:h-[512px] w-[350px] h-[350px] rounded-2xl overflow-hidden grid place-items-center bg-gray-50 bg-opacity-25">
            {isRunningPrediction && (
              <div>
                <Spinner />
                <p className="text-center mt-4 opacity-75">{status}</p>
              </div>
            )}
